...
export async function getAllProker(): Promise<ManagedProker[]> {
  if (DUMMY_DATA) {
  ...
  return ((data ?? []) as unknown as Row[]).map((row) => ({
    ...
    members: [...row.members]
      ...
      .map((m) => ({
        ...
        name: one(m.profile)?.full_name ?? nameById.get(m.profile_id) ?? "Pengurus",
        role: m.role,
      })),
  }));
}
...
export async function getPengurusOptions(): Promise<PengurusOption[]> {
  ...
  return (
    (data as { id: string; full_name: string; division_name: string | null }[] | null) ??
    []
  ).map((p) => ({ id: p.id, fullName: p.full_name, division: p.division_name }));
}
